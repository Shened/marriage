            ...
            <table>
                ...
                <tbody>
                    <tr v-for="confirmacao in confirmacoes.data" :key="confirmacao.id">
                        ...
                        <td>
                            <v-btn @click="showDetails(confirmacao)" class="btn"
                                prepend-icon="mdi-alert-circle-outline">
                                Ver Detalhes
                            </v-btn>
                        </td>
                        <td>
                            <v-btn @click="deleteConfirmacao(confirmacao.id)" class="btn"
...
const loadStats = async () => {
    try {
        const response = await axios.get('/confirmacoes/stats/all');
        if (response.data.success) Object.assign(stats, response.data.data);
    } catch (err) {
        console.error('Erro ao carregar estatísticas:', err);
    }
};

// Carrega confirmações (com filtros e paginação)
const loadConfirmacoes = async (page = 1) => {
    loading.value = true;
    try {
        const params = { page, presenca: filters.presenca, busca: filters.busca };
        const response = await axios.get('/confirmacoes', { params });
        if (response.data.success) confirmacoes.value = response.data.data;
    } catch (err) {
...
// Mostra detalhes de uma confirmação
...
const deleteConfirmacao = async (id) => {
    if (!confirm("Tens a certeza que queres remover esta confirmação?")) return;

    try {
        await axios.delete(`/confirmacoes/${id}`); // Endpoint DELETE
        // Atualiza lista após remoção
        loadConfirmacoes(confirmacoes.value.current_page);
        // Opcional: atualizar estatísticas
        loadStats();
    } catch (err) {
